 ...
 */
package featureextractionfinal.driving;
 ...
 * @author Ivan
 */
public class FeatureExtractionAccelerometerMagnetometerDistance {
    ...
    public static String getRow(String folder, String filename) {
        ...
        try {
            ...
            FileReader fileReader = new FileReader("data/driving/" + folder + "/" + filename + ".txt");
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i <= 2162; i++) {

            String line_acc = getRow(i + "", "accelerometer");
            String line_mag = getRow(i + "", "magnetometer");
            String line_dist = getDistance(i + "", "location");

            if (Double.parseDouble(line_dist) > 0) {

                String line_to_file = line_acc + "\t" + line_mag + "\t" + line_dist;

                try (FileWriter fw = new FileWriter("results/driving/accelerometer_magnetometer_distance.txt", true);
                        BufferedWriter bw = new BufferedWriter(fw);
                        PrintWriter out = new PrintWriter(bw)) {
                    out.println(line_to_file);
                } catch (IOException e) {
                    Logger.getLogger(FeatureExtractionAccelerometerMagnetometerDistance.class.getName()).log(Level.SEVERE, null, e);
                }
            }
        }
